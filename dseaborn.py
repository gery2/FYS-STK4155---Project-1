...
traindata = np.zeros((20, 20))
testdata = np.zeros((20, 20))

for j in range(20):
    X = create_X(x, y, n=j, intercept=False)
    ...
    for i in range(nlambdas):
        ...
        traindata[j][i] = MSE(z_train,zpredictRidge)
        testdata[j][i] = MSE(z_test,ztildeRidge)
# Now plot the results

fig, ax = plt.subplots(figsize = (10, 10))
sns.heatmap(testdata, annot=True, ax=ax, cmap="viridis")
ax.set_title("lambdas vs complexity, testdata")
ax.set_ylabel("$\eta$")
